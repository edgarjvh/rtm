...
class SettingsController extends Controller
{
    ...
    public function setSendingRatingEmailsAttended(Request $request){
        $status = $request->status;

        $setting = Setting::where('user_id', Auth::user()->id)->first();

        if ($setting){
            $setting->sending_rating_emails_not_hosted = $status == 'true' ? 1 : 0;
            $setting->save();
        }else{
            Setting::create([
                'user_id' => Auth::user()->id,
                'sending_rating_emails_not_hosted' => $status == 'true' ? 1 : 0
            ]);
        }
